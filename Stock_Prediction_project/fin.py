...
def load_models():
    try:
        st.write("Current Working Directory:", os.getcwd())
        lstm = load_model('/mount/src/skmart/lstm_model_all.h5', custom_objects={'mse': keras.losses.MeanSquaredError()})
        xgb = joblib.load('/mount/src/skmart/xgb_model_all.pkl')
        scalers = joblib.load('/mount/src/skmart/scalers.pkl')
        scaler_xgb = joblib.load('/mount/src/skmart/scaler_xgb.pkl')
...
def load_data():
    try:
        company_df = pd.read_csv('/mount/src/skmart/EQUITY_L.csv')
        price_df = pd.read_csv('/mount/src/skmart/all_stocks_data.csv')
